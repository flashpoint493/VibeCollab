...
class ClaudeAdapter(BaseIDEAdapter):
    ...
    ide_type = IDEType.CLAUDE
    display_name = "Claude Code"
    description = "Claude Code CLI with skill support"

    supports_skill = True
    skill_file_path = "CLAUDE.md"

    supports_mcp = False

    def get_skill_content(self) -> str:
        """获取 Claude Code Skill 文件内容。"""
        return """---
name: vibecollab
...
"""
    ...
    def get_mcp_config(self, command: str, args: list[str]) -> dict[str, Any]:
        ...
        Raises:
            NotImplementedError: Claude Code 暂不支持 MCP
        """
        raise NotImplementedError("Claude Code does not support MCP configuration yet")
