...
CodeBuddy 适配器实现，支持 Skill 注入和 MCP 配置。
"""
...
@register_adapter
class CodeBuddyAdapter(BaseIDEAdapter):
    """CodeBuddy IDE 适配器。"""

    ide_type = IDEType.CODEBUDDY
    display_name = "CodeBuddy"
    description = "CodeBuddy IDE with skills and MCP support"

    supports_skill = True
    skill_file_path = ".codebuddy/skills/vibecollab.md"

    supports_mcp = True
    mcp_config_path = ".mcp.json"

    def get_skill_content(self) -> str:
        """获取 CodeBuddy Skill 文件内容。"""
        return """# VibeCollab Skill for CodeBuddy
