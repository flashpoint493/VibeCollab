...
Trae 适配器实现，支持 Skill 注入（纯 Markdown 格式）。
"""
...
@register_adapter
class TraeAdapter(BaseIDEAdapter):
    """Trae IDE 适配器。"""

    ide_type = IDEType.TRAE
    display_name = "Trae"
    description = "Trae IDE with skills support (Markdown format)"

    supports_skill = True
    skill_file_path = ".trae/Skills/vibecollab.md"

    supports_mcp = False
    mcp_config_path = None

    def get_skill_content(self) -> str:
        """获取 Trae Skill 文件内容（纯 Markdown 格式，无 frontmatter）。"""
        return """# VibeCollab AI Collaboration Protocol
