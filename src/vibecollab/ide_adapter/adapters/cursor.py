...
Cursor 适配器实现，支持 Skill 注入和 MCP 配置。
"""
...
class CursorAdapter(BaseIDEAdapter):
    ...
    display_name = "Cursor"
    description = "Cursor IDE with rules-based skills and MCP support"

    supports_skill = True
    skill_file_path = ".cursor/rules/vibecollab.mdc"

    supports_mcp = True
    mcp_config_path = ".cursor/mcp.json"

    def get_skill_content(self) -> str:
        """获取 Cursor Skill 文件内容（.mdc 格式）。"""
        return """---
